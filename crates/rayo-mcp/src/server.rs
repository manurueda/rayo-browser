...
impl RayoServer {
    ...
    fn tool_definitions() -> Vec<Tool> {
        vec![
            ...
            Tool::new(
                "rayo_report",
                "Get collected tool errors for bug reporting. Returns structured error records (tool, params, error message, timestamp, version) ready for GitHub issue filing. Actions: get (return errors, optional last_n to limit count), clear (reset error log). Use this after tool failures to file an issue on manurueda/rayo-browser.",
                json_schema(json!({
                    "type": "object",
                    "properties": {
                        "action": { "type": "string", "enum": ["get", "clear"], "default": "get" },
                        "last_n": { "type": "integer", "description": "Return only the last N errors (default: all)" }
                    }
                })),
            ),
        ]
...
pub async fn run() -> Result<()> {
    ...
    tracing::info!(
        "rayo-mcp v{} starting on stdio (9 tools)",
