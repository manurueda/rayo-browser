...
/// Generate a description from a suite name and URL (used when DetectedFlow is unavailable).
///
/// This is a fallback for the scan command, which runs YAML suites generated by discover
/// but doesn't have access to the original DetectedFlow objects.
pub fn describe_flow_from_name(suite_name: &str, url: &str) -> String {
    let url_path = extract_url_path(url);
    let lower = suite_name.to_lowercase();

    if lower.contains("login") || lower.contains("auth") || lower.contains("signin") {
        format!(
            "Login: visits {}, enters credentials, clicks submit",
            html_escape(&url_path)
        )
    } else if lower.contains("search") {
        format!(
            "Search: visits {}, performs search, verifies results",
            html_escape(&url_path)
        )
    } else if lower.contains("crud") {
        format!(
            "Data: creates entry on {}, verifies list updates",
            html_escape(&url_path)
        )
    } else if lower.contains("navigation") || lower.contains("nav") || lower.contains("smoke") {
        format!(
            "Navigation: visits {}, checks that pages load correctly",
            html_escape(&url_path)
        )
    } else if lower.contains("form") {
        let page = page_name_from_path(&url_path);
        format!(
            "{} Form: fills out fields, submits form",
            html_escape(&page)
        )
    } else {
        format!(
