...
rayo-ui's discover system detects flows per-page but cannot map multi-page user journeys. Apps with paywalls, auth gates, and tiered access need persona-aware crawling that shows where different user types diverge. Without this, there's no way to generate E2E tests that cover the full user experience across subscription tiers.
